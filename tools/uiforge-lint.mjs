...
const RULES = [
  ...
  { id: 'hype-copy', sev: 'BLOCKER',
    ...
    run: () => scan(HYPE) },
  { id: 'raw-hex', sev: 'WARN',
    why: 'Raw hex/rgb at point of use in component code — nothing traces to a token. Define semantic tokens and use var(--…). (Hex in a CSS token file is fine.)',
    run: () => scan(/['"`]#[0-9a-fA-F]{6}\b|:\s*#[0-9a-fA-F]{6}\b(?![^}]*\{)|rgba?\(\s*\d/, { codeOnly: true }) },
